# MFSD Weekly RAG — Technical Specification v1.1
...
> **⚠️ Outstanding (as of v7.3.0): not yet migrated to the SteveGPT v8 pattern.** Every AI call below still uses the legacy `$GLOBALS['mwai']->simpleTextQuery()` (AI Engine), guarded by `isset($GLOBALS['mwai'])`. These calls only work while AI Engine is active; if it is deactivated they silently return fallbacks. Migration target: register integration slots via `stevegpt_plugin_integration_slots`, read chatbot IDs from `mfsd_stevegpt_map_rag_*` options, and call `SteveGPT_Chatbot::get($id)->render_prompt()` → `query($prompt, $user_id)` per `stevegtp/techspecs/MFSD_CHATBOT_IMPLEMENTATION_PATTERN.md`.
...
| Version | Changes |
|---------|---------|
| 7.3.0 | Current. REST namespace moved from `mfsd/v1` to `mfsd-rag/v1` (all 12 routes, localized `MFSD_RAG_CFG` URLs and the two admin reset endpoints) to stop the Supabase bridge JWT middleware returning HTTP 401 on every RAG call. No DB or JS changes. |
