# MFSD Weekly RAG — Technical Specification v1.2
...
5. The JS `renderIntro()` function immediately calls the `/mfsd-rag/v1/status` REST endpoint. Depending on status:
...
1. Questions are fetched from `/mfsd-rag/v1/questions?week=N`. The PHP interleaves RAG and MBTI questions (2 RAG, 1 MBTI, repeat).
...
3. Clicking any answer button POST to `/mfsd-rag/v1/answer`. The PHP writes to the appropriate answers table and, on first answer, marks the ordering task as `in_progress`.
4. **Guidance flow:** Clicking "Ask SteveGPT" sends a POST to `/mfsd-rag/v1/question-guidance`. The PHP queries previous weeks' answers for context, then calls `$GLOBALS['mwai']->simpleTextQuery($prompt)` to get coaching text. A TTS control (speaker + stop buttons) is attached to the guidance text.
5. **Previous-answer strip:** `/mfsd-rag/v1/previous-answer` fetches prior weeks' answers for the same question to show a "trend" indicator.
...
2. POST to `/mfsd-rag/v1/red-suggestions` generates (via SteveGPT) an intro message and three practical improvement suggestions, taking into account any previous Red plan for the same question.
...
5. A mini chat window ("Ask SteveGPT for more ideas") allows follow-up questions via `/mfsd-rag/v1/question-chat?is_red_followup=true`.
6. Submitting the plan POSTs to `/mfsd-rag/v1/red-plan` which saves it to `wp_mfsd_rag_red_plans`.
...
1. After all questions are answered (or on revisit of a completed week), POST to `/mfsd-rag/v1/summary`.
...
When a student starts a new week (status `not_started`, week > 1), the `/mfsd-rag/v1/status` response includes `previous_week_summary` and an `intro_message` generated by SteveGPT referencing the prior week's RAG and MBTI results.
...
All routes use namespace `mfsd-rag/v1` (moved from the shared `mfsd/v1` namespace in v7.3.0 — see note below). All student-facing routes require login (`check_permission()`). POST/PUT/DELETE additionally verify `X-WP-Nonce` header against the `wp_rest` nonce. Admin-only routes require `manage_options`.

| Route | Method | Auth | Description |
|-------|--------|------|-------------|
| `/mfsd-rag/v1/questions` | GET | Login | Returns questions for a given week, interleaved RAG then MBTI (2:1 ratio). Accepts `?week=N`. |
| `/mfsd-rag/v1/answer` | POST | Login + nonce | Saves a single answer. Body: `week`, `question_id`, `rag` (R/A/G for RAG/MBTI) or `disc_answer` (1–5 for DISC). Also marks ordering task `in_progress` on first answer. |
| `/mfsd-rag/v1/summary` | POST | Login + nonce | Aggregates results, calculates MBTI/DISC, generates AI summary via SteveGPT, saves to `week_summaries`, marks task `completed`. Respects summary caching option. |
| `/mfsd-rag/v1/status` | GET | Login | Returns completion status, answer counts vs expected, answered question IDs, pending Red plan IDs, previous week summary, and AI intro message. Accepts `?week=N`. |
| `/mfsd-rag/v1/previous-answer` | GET | Login | Returns this student's prior answers for a given `question_id` in all weeks before `week`. |
| `/mfsd-rag/v1/question-guidance` | POST | Login + nonce | Generates SteveGPT guidance for a specific question, including trend context and previous Red plan. |
| `/mfsd-rag/v1/all-weeks-summary` | GET | Login | Returns completion status and RAG/MBTI results for all 6 weeks for the current student. Used for the week-tab review screen. |
| `/mfsd-rag/v1/question-chat` | POST | Login + nonce | Single-turn chat with SteveGPT in context of a question. `is_red_followup=true` uses a Red-specific system prompt. |
| `/mfsd-rag/v1/red-suggestions` | POST | Login + nonce | Generates SteveGPT intro + 3 practical suggestions for a Red answer. Includes previous plan context. Returns `steve_intro`, `suggestions[]`, `prev_plans[]`, `prev_answer`, `word_target`. |
| `/mfsd-rag/v1/red-plan` | POST | Login + nonce | Saves or replaces a Red improvement plan to `mfsd_rag_red_plans`. |
| `/mfsd-rag/v1/admin-reset-week` | POST | `manage_options` | Deletes all answers, MBTI/DISC results, red plans, and cached summary for a given `user_id` + `week`. |
| `/mfsd-rag/v1/admin-reset-cm-progress` | POST | `manage_options` | Deletes a specific ordering progress record from `wp_mfsd_task_progress` for a given `user_id` + `task_slug`. |

> **Namespace note (v7.3.0):** Until v7.2.0 these routes lived under `mfsd/v1`, shared with `mfsd-supabase-bridge` and `mfsd-wealth-happiness`. The Supabase bridge's JWT middleware (`rest_authentication_errors`) matches every request under `/mfsd/v1/` by URL prefix and, in bridge v7.0.0, rejected any request without an `Authorization: Bearer` header with `401 rest_forbidden — "Missing or invalid Authorization header"`. RAG authenticates with WP cookie + `X-WP-Nonce`, so every RAG call (questions, status, etc.) failed with HTTP 401 for logged-in students. Fixed by (a) moving RAG to its own `mfsd-rag/v1` namespace (this plugin, v7.3.0) and (b) making the bridge fall through to cookie auth when no Bearer token is sent (bridge v7.1.0). **Rule going forward: every plugin registers its own namespace — do not add routes to `mfsd/v1`.** All URLs reach the JS via `MFSD_RAG_CFG` (`wp_localize_script`), so the JS itself has no hard-coded namespace.
...
Selects a student and week, then POSTs to `/mfsd-rag/v1/admin-reset-week` via fetch with the REST nonce. On success, displays deleted record count. Permanently removes all answers, MBTI/DISC results, red plans, and cached summary for that student/week.
...
- Provides a separate reset widget for ordering progress only (does not remove RAG answer data) via `/mfsd-rag/v1/admin-reset-cm-progress`.

---

## SteveGPT Integration

All AI calls use the SteveGPT v8 pattern (`stevegtp/techspecs/MFSD_CHATBOT_IMPLEMENTATION_PATTERN.md`). As of v7.4.0 there are no `$GLOBALS['mwai']` / `simpleTextQuery()` calls; the plugin no longer depends on AI Engine.

**Integration slots** — registered via `stevegpt_plugin_integration_slots` ("Weekly RAG" tab in SteveGPT → Chatbots):

| Slot (role) | Option key | Mode | Endpoint | Tokens |
|---|---|---|---|---|
| RAG chat assistant | `mfsd_stevegpt_map_rag_chat` | Chat (widget) | Shortcode render | — |
| Week intro | `mfsd_stevegpt_map_rag_week_intro` | Task | `api_status` | `student_name`, `age_desc`, `week`, `previous_week`, `greens`, `ambers`, `reds`, `personality` |
| Question guidance | `mfsd_stevegpt_map_rag_guidance` | Task | `api_question_guidance` | `student_name`, `age_desc`, `question_text`, `question_type`, `previous_answers`, `previous_red_plan` |
| Red plan suggestions | `mfsd_stevegpt_map_rag_red_suggestions` | Task | `api_red_suggestions` | `student_name`, `age_desc`, `question_text`, `last_plan`, `previous_answer` — **output must keep the `INTRO:` / `SUGGESTION_1-3:` line format** (parsed server-side) |
| Question chat | `mfsd_stevegpt_map_rag_question_chat` | Task | `api_question_chat` | `student_name`, `age_desc`, `week`, `question_text`, `question_type`, `previous_answers`, `message` |
| Red plan chat | `mfsd_stevegpt_map_rag_red_chat` | Task | `api_question_chat` (`is_red_followup=true`) | same as Question chat |
| Week summary | `mfsd_stevegpt_map_rag_summary` | Task | `api_summary` | `student_name`, `age_desc`, `week`, `reds`, `ambers`, `greens`, `total_score`, `personality`, `communication_style`, `dream_jobs`, `plan_context` |

**`steve_task(string $slot, string $fallback_prompt, array $tokens, int $user_id): string`** (private):
1. Reads `mfsd_stevegpt_map_rag_{$slot}`. Returns `''` if the slot is empty or `SteveGPT_Chatbot` is unavailable.
2. If the chatbot has a `prompt_template`, the prompt is `render_prompt($tokens)`; otherwise the plugin's built-in prompt (unchanged from pre-7.4.0) is sent.
3. Appends `NO_CODES_RULE` ("Never mention MBTI, Myers-Briggs, DISC, or any letter-code personality labels… refer to personality only by the name given") to **every** prompt, templated or built-in.
4. `query($prompt, $user_id)` — skills, student context and cost logging handled by SteveGPT. Errors are logged as `[MFSD RAG] SteveGPT task error (<slot>): …` and return `''`.

**Fallbacks when a slot is empty or errors:** week intro → no intro message; guidance → empty; red suggestions → static intro + 3 static suggestions; chat → "I'm having trouble connecting right now. Please try again."; summary → no AI text, so the summary is **not saved** and the ordering task is **not marked complete**. Assign the Week summary slot before deploying.

| Context | Key prompt elements (built-in prompt) |
|---------|---------------------|
| Week intro | Previous week's R/A/G counts, MBTI type, week number, student name, age description |
| Per-question guidance | Question text and type, previous weeks' answers, previous Red plan for the same question |
| Red suggestions | Question text, previous Red plan context, whether Red again or improved |
| Red plan follow-up chat | Question text, Red context, student message |
| Per-question chat | Question text, type, previous answers, student message |
| Week summary | Full RAG/MBTI/DISC results, previous weeks, dream jobs, Red plan outcomes, Solutions Mindset principles |

**Age-aware prompting:** all prompts include `"aged {N}"` (from `pp_dob`) or the `"aged 11-14"` fallback, which also sets the Red plan word target in `age-specific` mode.

**No test names or codes reach the AI (v7.4.0).** Students must never see MBTI/DISC terminology or letter codes, so the plugin translates them before building any prompt:

| Source | Sent to the AI as | Helper |
|---|---|---|
| 4-letter type (`type4`, e.g. `INFJ`) | `The Advocate (Diplomat family)` | `personality_label()` via `PERSONALITY_NAMES` (mirrors `mfsd-personality-test`) |
| DISC type (e.g. `DI`) | `direct and results-focused, and also outgoing and enthusiastic` | `communication_style_label()` via `COMMUNICATION_STYLES` (D direct/results-focused, I outgoing/enthusiastic, S steady/supportive, C careful/detail-focused) |

Prompt wording changed to match: MBTI questions are described as "personality questions"; the summary asks Steve to "reflect on their personality type and communication style (by name only)". DISC percentages are no longer sent. Unknown or missing codes produce an empty label and the line is left out.
...
**Week tabs:** On the summary screen, tabs for weeks 1–6 are rendered; clicking a tab calls the `/mfsd-rag/v1/all-weeks-summary` endpoint and re-renders the summary for that week.
...
|-----------|------|-------|
| SteveGPT (`stevegtp`, `SteveGPT_Chatbot`) | Soft required | All AI via integration slots (v7.4.0+). Features degrade to static fallbacks if SteveGPT is inactive or a slot is unassigned. AI Engine is no longer used. |
...
| Version | Changes |
|---------|---------|
| 7.4.0 | Current. SteveGPT migration: all 5 legacy `$GLOBALS['mwai']->simpleTextQuery()` calls (week intro, guidance, red suggestions, question/red chat, week summary) moved to `SteveGPT_Chatbot::get()->query()` via new `steve_task()` helper; 6 new integration slots registered (`rag_week_intro`, `rag_guidance`, `rag_red_suggestions`, `rag_question_chat`, `rag_red_chat`, `rag_summary`). Built-in prompts used when a chatbot has no template. **No test names or codes sent to the AI:** 4-letter types mapped to Who Am I names, DISC letters to plain-English communication styles, MBTI questions called "personality questions", DISC percentages dropped, and `NO_CODES_RULE` appended to every prompt. Summary slot tokens `personality` + `communication_style` replace `disc_type`; week intro gains `personality`. No DB, REST or JS changes. |
| 7.3.0 | REST namespace moved from `mfsd/v1` to `mfsd-rag/v1` (all 12 routes, localized `MFSD_RAG_CFG` URLs and the two admin reset endpoints) to stop the Supabase bridge JWT middleware returning HTTP 401 on every RAG call. No DB or JS changes. |
| 7.2.0 | MYF-247: replaced hardcoded corporate inline styles with gamer CSS classes. |
| 7.1.0 | Full DISC integration, Red plan follow-up flow with word-count progress bar, TTS/STT voice engine, age-aware prompting via ProfilePress DOB, ordering integration via `mfsd-ordering`, Course Management admin tab, admin-reset-cm-progress endpoint |
